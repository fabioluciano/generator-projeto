    ...
    module.exports = yeoman.generators.Base.extend({
        initializing: function () {
            ...
            that.argument('module', {
                required: false,
                type: String,
                desc: 'O módulo onde a fectory residirá'
            });

            that.argument('name', {
                required: false,
                type: String,
                desc: 'O nome da factory'
            });
        ...
        prompting : function() {
            ...
            if(that.module === undefined) {
                questions.push({
                    type: "list",
                    name: "module",
                    message: "A factory deverá ser criado em qual módulo?",
            ...
            if (that.name === undefined || that.name.length < 3) {
                questions.push({
                    type: 'input',
                    name: 'name',
                    message: 'Qual será o nome da factory?',
                    required: true,
        ...
        writting: function() {
            ...
            that.name       = that.name;
            that.filePath   = applicationStructure.modules + that.module + '/' + moduleStructure.factory;

            if (that.confirm) {
                that.template('factory.js', that.filePath + that.name.toLowerCase() + '-factory.js');
            }
