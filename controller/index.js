    ...
    module.exports = yeoman.generators.Base.extend({
        initializing: function () {
            ...
            that.argument('module', {
                required: false,
                type: String,
                desc: 'O módulo onde a controller residirá'
            });

            that.argument('name', {
                required: false,
                type: String,
                desc: 'O nome do controller'
            });
            ...
            var questions  = [],
                ...
                if(that.module === undefined) {
                    questions.push({
                        type: "list",
                        name: "module",
                        message: "O controller deverá ser criado em qual módulo?",
                ...
                if (that.name === undefined || that.name.length < 3) {
                    questions.push({
                        type: 'input',
                        name: 'name',
                        message: 'Qual será o nome da controller?',
                        required: true,
        ...
        writting: function() {
            ...
            that.name       = that.name;
            that.filePath   = applicationStructure.modules + that.module + '/' + moduleStructure.controller;

            if (that.confirm) {
                that.template('controller.js', that.filePath + that.name.toLowerCase() + '-controller.js');
            }
