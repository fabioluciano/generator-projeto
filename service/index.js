( function() {
    ...
    module.exports = yeoman.generators.Base.extend({
        initializing: function () {
          ...
          filesystem.readdir(applicationStructure.modules, (err, modules) => {
            ...
            that.argument('module', {
                required: false,
                type: String,
                desc: 'O módulo onde o service residirá'
            });

            that.argument('name', {
                required: false,
                type: String,
                desc: 'O nome do service'
            });
        ...
        prompting : function() {
            ...
            if(that.module === undefined) {
                questions.push({
                    type: "list",
                    name: "module",
                    message: "O service deverá ser criado em qual módulo?",
            ...
            if (that.name === undefined || that.name.length < 3) {
                questions.push({
                    type: 'input',
                    name: 'name',
                    message: 'Qual será o nome do service?',
                    required: true,
        ...
        writting: function() {
            ...
            that.name       = that.name;
            that.filePath   = applicationStructure.modules + that.module + '/' + moduleStructure.service;

            if (that.confirm) {
                that.template('service.js', that.filePath + that.name.toLowerCase() + '-service.js');
            }
