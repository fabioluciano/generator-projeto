    ...
    module.exports = yeoman.generators.Base.extend({
        initializing: function () {
            ...
            that.argument('module', {
                required: false,
                type: String,
                desc: 'O módulo onde a directive residirá'
            });

            that.argument('name', {
                required: false,
                type: String,
                desc: 'O nome da directive'
            });

            that.argument('type', {
                required: false,
                type: String,
                desc: 'O tipo da directive'
            });
        ...
        prompting : function() {
            ...
            if(that.module === undefined) {
                questions.push({
                    type: "list",
                    name: "module",
                    message: "A directive deverá ser criada em qual módulo?",
            ...
            if(that.type === undefined) {
                questions.push({
                    type: "list",
                    name: "type",
                    message: "Qual o tipo de directive será criada?",
                    choices: [
                        'Elemento',
                        'Atributo',
                        'Classe',
            ...
            if (that.name === undefined || that.name.length < 3) {
                questions.push({
                    type: 'input',
                    name: 'name',
                    message: 'Qual será o nome da directive?',
                    required: true,
            ...
            that.prompt(questions, function (props) {
                ...
                if('type' in props) {
                    that.type = props.type.toLowerCase();

                    switch(that.type) {
                        case 'elemento' :
                        case 'atributo' :
                        case 'classe' :
                            that.type = that.type.toUpperCase()[0];
                            break;
                        case 'comentário' :
                            that.type = 'M';
                            break;
        ...
        writting: function() {
            ...
            that.name       = that.name;
            that.filePath   = applicationStructure.modules + that.module + '/' + moduleStructure.directive;

            if (that.confirm) {
                that.template('directive.js', that.filePath + that.name.toLowerCase() + '-directive.js');
            }
